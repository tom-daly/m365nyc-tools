...
function testConfigurationSwitchingLogic() {
  ...
  let raffleState: { raffleStarted: boolean; winners: any[]; currentRound: number } = { raffleStarted: false, winners: [], currentRound: 0 };
  ...
  raffleState = { raffleStarted: false, winners: [{ team: 'Team A', round: 1, roundName: 'Round 1' }], currentRound: 1 };
  shouldShowWarning = raffleState.raffleStarted || raffleState.winners.length > 0 || raffleState.currentRound > 0;
